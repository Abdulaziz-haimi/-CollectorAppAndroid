...
class SessionManager(context: Context) {
    ...
    companion object {
        // غيّر هذا إلى IP اللابتوب والمنفذ الذي يعمل عليه الـ Backend عبر HTTPS
        private const val DEFAULT_BASE_URL = "https://192.168.0.135:8443/"
    }
    ...
    fun isLoggedIn(): Boolean {
        return !token.isNullOrBlank() && collectorId > 0
    }

    fun clear() {
        // نحافظ على baseUrl و deviceCode حتى لا يضطر المستخدم لإدخال الرابط كل مرة
        val savedBaseUrl = baseUrl
        val savedDeviceCode = deviceCode
    ...
    private fun normalizeBaseUrl(value: String): String {
        ...
        val withScheme = when {
            trimmed.startsWith("https://", ignoreCase = true) -> trimmed
            trimmed.startsWith("http://", ignoreCase = true) -> trimmed.replaceFirst(
                "http://",
                "https://",
                ignoreCase = true
            )
